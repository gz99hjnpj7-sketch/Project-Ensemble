...
export function ForecastChart({ compositeData, data, series = [] }: { compositeData?: CompositePoint[]; data?: CompositePoint[]; series?: Series[] }) {
  ...
  return <div style={{ width: "100%", height: 320 }}><ResponsiveContainer><LineChart data={chartData} margin={{ left: 8, right: 18, top: 12, bottom: 8 }}>
    ...
    <Line type="monotone" dataKey="composite" name="Composite" stroke="#67d59a" strokeWidth={3} dot={false} connectNulls />
    {series.map((item, index) => <Line key={index} type="monotone" dataKey={`s${index}`} name={item.name} stroke={colors[index % colors.length]} strokeWidth={1.5} strokeDasharray="4 2" dot={false} connectNulls />)}
