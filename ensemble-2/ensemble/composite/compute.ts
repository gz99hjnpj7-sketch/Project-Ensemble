...
export type CompositeResult = {
  ...
  sourceBreakdown: Array<CompositeInput & { weight: number; contribution: number | null; included: boolean }>;
};
...
export function computeCompositeForecast(inputs: CompositeInput[]): CompositeResult {
  ...
  const usable = inputs.filter((input) => includedIds.has(input.marketId));
  ...
  return {
    ...
    policy,
    sourceBreakdown: inputs.map((input) => {
