...
export type QualityScoreInput = Pick<NormalizedMarketInput, "liquidity" | "volume" | "bid" | "ask" | "lastUpdated" | "closeTime" | "resolutionStatus"> & { createdAt?: Date | null };
...
export function computeQualityScore(input: QualityScoreInput, now = new Date()): QualityScoreResult {
  ...
  const raw = liquidityScore * 0.28 + spreadScore * 0.24 + volumeScore * 0.22 + recencyScore * 0.18 + marketAgeScore * 0.08 - resolvedPenalty;
  const score = clamp(Math.round(raw), 0, 100);
  const flags = buildQualityFlags(input, { liquidityScore, spreadScore, volumeScore, recencyScore });
  return { score, confidence: score >= 75 ? SignalConfidence.HIGH : score >= 45 ? SignalConfidence.MEDIUM : SignalConfidence.LOW, flags, liquidityScore, spreadScore, volumeScore, recencyScore, marketAgeScore };
}

function scoreLogScale(value: number | null | undefined, low: number, high: number): number {
  if (!value || value <= 0) return 0;
  return clamp(Math.round(((Math.log10(value) - Math.log10(low)) / (Math.log10(high) - Math.log10(low))) * 100), 0, 100);
}
...
function buildQualityFlags(input: QualityScoreInput, scores: Pick<QualityScoreResult, "liquidityScore" | "spreadScore" | "volumeScore" | "recencyScore">): string[] {
  const flags: string[] = [];
